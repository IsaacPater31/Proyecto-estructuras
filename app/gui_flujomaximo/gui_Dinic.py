...
from algorithms.flujomaximo.Dinic import dinic
import matplotlib
...
class GrafoDinicApp(tk.Tk):
    def __init__(self, G, nodos):
        super().__init__()
        self.title("Dinic: Flujo máximo entre dos nodos")
        ancho, alto = 1400, 750
    ...
    def _crear_layout(self):
        ...
        ttk.Button(self.left, text="Calcular Flujo Máximo (Dinic)", command=self.calcular_flujo_maximo).pack(pady=20, fill=tk.X)
    ...
    def calcular_flujo_maximo(self):
        ...
        try:
            self.resultado_flujo = dinic(G_redirigido, fuente, sumidero)
    ...
    def mostrar_resultados_flujo(self, fuente, sumidero):
        ...
        self.resultado.insert(tk.END, f"Etapas de aumento encontradas: {len(self.resultado_flujo['flow_paths'])}\n\n")
        self.resultado.insert(tk.END, "AUMENTOS DE FLUJO:\n")
        self.resultado.insert(tk.END, "-" * 58 + "\n")
        for i, path_info in enumerate(self.resultado_flujo['flow_paths'], 1):
            flow = path_info['flow']
            self.resultado.insert(tk.END, f"{i}. Flujo aumentado: {flow:.2f} unidades\n")
