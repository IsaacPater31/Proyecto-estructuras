...
from algorithms.flujomaximo.Edmonds_Karp import edmonds_karp
import matplotlib
...
class GrafoEdmondsKarpApp(tk.Tk):
    def __init__(self, G, nodos):
        super().__init__()
        self.title("Edmonds-Karp: Flujo máximo entre dos nodos")
        ancho, alto = 1400, 750
    ...
    def _crear_layout(self):
        ...
        ttk.Button(self.left, text="Calcular Flujo Máximo (Edmonds-Karp)", command=self.calcular_flujo_maximo).pack(pady=20, fill=tk.X)
    ...
    def calcular_flujo_maximo(self):
        ...
        try:
            self.resultado_flujo = edmonds_karp(G_redirigido, fuente, sumidero)
